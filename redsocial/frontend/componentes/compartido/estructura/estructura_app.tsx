...
const CLAVE_SIDEBAR_COLAPSADO = 'codeplex-sidebar-colapsado'
const CONSULTA_MOVIL = '(max-width: 800px)'

export function EstructuraApp({ children, paginaActiva, alturaCompleta }: EstructuraAppProps) {
  const [colapsado, setColapsado] = useState(() => {
    try {
      if (window.matchMedia(CONSULTA_MOVIL).matches) return false
      return sessionStorage.getItem(CLAVE_SIDEBAR_COLAPSADO) === '1'
    } catch {
      return false
    }
  })
  ...
  function alAlternarSidebar() {
    ...
    setColapsado((actual) => {
      const nuevo = !actual
      try {
        sessionStorage.setItem(CLAVE_SIDEBAR_COLAPSADO, nuevo ? '1' : '0')
      } catch {
      }
      return nuevo
    })
  ...
  return (
    <div className="flex min-h-screen flex-col bg-fondo">
      <BarraSuperior onAlternarSidebar={alAlternarSidebar} colapsado={colapsado} />
      <div className="flex flex-1">
        ...
        <main
          className={
            ...
            (alturaCompleta ? 'flex h-[calc(100dvh-64px)] min-h-0 overflow-hidden' : '')
          }
        >
          {children}
        </main>
