...
import { PARTICIPANTES, REUNION_EN_CURSO, CONTEO_PARTICIPANTES_EN_CURSO } from '../../datos/mensajeria/videollamadas'
      ...
      <div className="-mx-6 -mt-6.25 -mb-10.5 flex h-[calc(100vh-64px)] min-w-0 flex-col gap-3.5 px-5 pb-5 pt-5 max-[800px]:-mx-3 max-[800px]:-mt-4.5 max-[800px]:-mb-4.5">
        <header className="flex flex-wrap items-center gap-4 rounded-xl border border-borde bg-white px-4 py-3">
          <div className="min-w-[200px] flex-1">
            <strong className="block text-[13.5px] text-texto">{REUNION_EN_CURSO.titulo}</strong>
            <p className="m-0 mt-0.5 flex flex-wrap items-center gap-2 text-[11.5px] text-texto-suave">
              ...
              <span className="inline-flex items-center gap-1.25 text-exito">
                <span className="h-[7px] w-[7px] rounded-full bg-exito" /> {catalogoMensajeria.leyendas.en_llamada}
              </span>
              <span className="[font-variant-numeric:tabular-nums]">{REUNION_EN_CURSO.duracion}</span>
            </p>
          </div>
          <div className="flex flex-none items-center gap-2">
            <button type="button" aria-label={catalogoMensajeria.botones.ver_participantes} onClick={() => setParticipantesAbiertos(true)} className="inline-flex h-8.5 items-center gap-1.5 rounded-lg border-0 bg-primario-suave px-3 text-[12.5px] font-bold text-primario">
              <Icono name="usuarios" className="h-[15px] w-[15px]" /> {CONTEO_PARTICIPANTES_EN_CURSO.total}
            </button>
            <button type="button" aria-label={catalogoMensajeria.botones.chat} className="grid h-8.5 w-8.5 flex-none place-items-center rounded-lg border-0 bg-[#f6f4ff] text-texto">
              <Icono name="comentario" className="h-4 w-4" />
            </button>
            <button type="button" aria-label={catalogoMensajeria.botones.seguridad_llamada} className="grid h-8.5 w-8.5 flex-none place-items-center rounded-lg border-0 bg-[#f6f4ff] text-texto">
              <Icono name="escudo" className="h-4 w-4" />
            </button>
            <button type="button" aria-label={catalogoMensajeria.botones.grabando_llamada} className="grid h-8.5 w-8.5 flex-none place-items-center rounded-lg border-0 bg-[#fdecec] text-peligro">
              <Icono name="punto-circular" className="h-4 w-4" />
            </button>
            <button type="button" aria-label={catalogoMensajeria.botones.mas_opciones} className="grid h-8.5 w-8.5 flex-none place-items-center rounded-lg border-0 bg-[#f6f4ff] text-texto">
          ...
          <a href="12-04-mensajes-04-videollamadas.html" className="inline-flex h-9.5 flex-none items-center gap-1.75 whitespace-nowrap rounded-lg bg-peligro px-4 text-[13px] font-bold text-white no-underline">
            <Icono name="llamada" className="h-[15px] w-[15px] rotate-[135deg]" /> {catalogoMensajeria.botones.salir_de_la_llamada}
          </a>
        </header>
        ...
        <div className="relative flex min-h-0 flex-1 gap-4">
          <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-2.5">
            <div className="grid min-h-0 flex-1 auto-rows-[minmax(120px,1fr)] grid-cols-[repeat(auto-fill,minmax(170px,1fr))] gap-3 overflow-y-auto rounded-[14px] bg-[#0f0c1e] p-4 max-[780px]:grid-cols-[repeat(auto-fill,minmax(130px,1fr))] max-[480px]:grid-cols-[repeat(auto-fill,minmax(105px,1fr))] max-[480px]:gap-2">
              {PARTICIPANTES.map((p) => (
                <article key={p.nombre} className="relative min-h-[120px] overflow-hidden rounded-[10px] bg-[linear-gradient(155deg,#362a63,#1c1735)]">
                  {p.organizador && (
                    <span className="absolute left-2 top-2 z-[1] rounded-md bg-[rgba(15,12,30,.55)] px-2.25 py-0.75 text-[10px] font-bold text-white">{catalogoMensajeria.leyendas.organizador}</span>
                  )}
            ...
            <div className="flex max-w-full flex-none items-center gap-1 self-center overflow-x-auto rounded-[14px] bg-[rgba(15,12,30,.9)] px-2.5 py-2">
              <button type="button" onClick={() => setMicActivo((v) => !v)} className="flex flex-none flex-col items-center gap-1 rounded-[10px] border-0 bg-transparent px-2.5 py-1.5 text-white hover:bg-white/[.08]">
                ...
                <small className="whitespace-nowrap text-[10px]">{catalogoMensajeria.botones.microfono}</small>
              </button>
              <button type="button" onClick={() => setCamaraActivo((v) => !v)} className="flex flex-none flex-col items-center gap-1 rounded-[10px] border-0 bg-transparent px-2.5 py-1.5 text-white hover:bg-white/[.08]">
                ...
                <small className="whitespace-nowrap text-[10px]">{catalogoMensajeria.botones.camara}</small>
              </button>
              <button type="button" className="flex flex-none flex-col items-center gap-1 rounded-[10px] border-0 bg-transparent px-2.5 py-1.5 text-white hover:bg-white/[.08]">
                ...
                <small className="whitespace-nowrap text-[10px]">{catalogoMensajeria.botones.compartir_pantalla}</small>
              </button>
              <button type="button" onClick={() => setParticipantesAbiertos(true)} className="flex flex-none flex-col items-center gap-1 rounded-[10px] border-0 bg-transparent px-2.5 py-1.5 text-white hover:bg-white/[.08]">
                <span className="relative flex items-center">
                  ...
                  <span className="absolute -right-2.5 -top-1.5 grid h-[15px] min-w-[15px] place-items-center rounded-lg bg-primario px-0.75 text-[9px] font-bold text-white">{CONTEO_PARTICIPANTES_EN_CURSO.total}</span>
                </span>
                <small className="whitespace-nowrap text-[10px]">{catalogoMensajeria.botones.participantes}</small>
              </button>
              <button type="button" className="flex flex-none flex-col items-center gap-1 rounded-[10px] border-0 bg-transparent px-2.5 py-1.5 text-white hover:bg-white/[.08]">
                <Icono name="comentario" className="h-[19px] w-[19px]" />
                <small className="whitespace-nowrap text-[10px]">{catalogoMensajeria.botones.chat}</small>
              </button>
              <button type="button" className="flex flex-none flex-col items-center gap-1 rounded-[10px] border-0 bg-transparent px-2.5 py-1.5 text-white hover:bg-white/[.08]">
                <Icono name="sentimiento" className="h-[19px] w-[19px]" />
                <small className="whitespace-nowrap text-[10px]">{catalogoMensajeria.botones.reacciones}</small>
              </button>
              <button type="button" className="flex flex-none flex-col items-center gap-1 rounded-[10px] border-0 bg-transparent px-2.5 py-1.5 text-white hover:bg-white/[.08]">
                <Icono name="puntos" className="h-[19px] w-[19px]" />
                <small className="whitespace-nowrap text-[10px]">{catalogoMensajeria.botones.mas}</small>
              </button>
              <a href="12-04-mensajes-04-videollamadas.html" aria-label={catalogoMensajeria.botones.salir_de_la_llamada} className="ml-2 grid h-[46px] w-[46px] flex-none place-items-center rounded-full bg-peligro text-white">
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              <h2 className="m-0 text-sm font-bold text-texto">{catalogoMensajeria.botones.participantes} ({CONTEO_PARTICIPANTES_EN_CURSO.total})</h2>
              <button type="button" aria-label={catalogoMensajeria.botones.cerrar} onClick={() => setParticipantesAbiertos(false)} className="grid h-6.5 w-6.5 place-items-center rounded-md border-0 bg-transparent text-texto-suave">
            ...
            <div className="mb-2.5 flex gap-1.5 overflow-x-auto">
              <button type="button" className="inline-flex flex-none items-center gap-1 whitespace-nowrap rounded-[20px] border border-primario bg-primario px-2.75 py-1.25 text-[11px] font-medium text-white">
                {catalogoMensajeria.filtros.todos} <span className="text-[10px]">{CONTEO_PARTICIPANTES_EN_CURSO.todos}</span>
              </button>
              <button type="button" className="inline-flex flex-none items-center gap-1 whitespace-nowrap rounded-[20px] border border-borde bg-white px-2.75 py-1.25 text-[11px] font-medium text-texto-suave">
                {catalogoMensajeria.filtros.en_linea} <span className="text-[10px]">{CONTEO_PARTICIPANTES_EN_CURSO.enLinea}</span>
              </button>
              <button type="button" className="inline-flex flex-none items-center gap-1 whitespace-nowrap rounded-[20px] border border-borde bg-white px-2.75 py-1.25 text-[11px] font-medium text-texto-suave">
                {catalogoMensajeria.filtros.invitados} <span className="text-[10px]">{CONTEO_PARTICIPANTES_EN_CURSO.invitados}</span>
              </button>
              <button type="button" aria-label={catalogoMensajeria.botones.mas_filtros} className="grid h-6.5 w-6.5 flex-none place-items-center rounded-[20px] border border-borde bg-white text-[11px] font-medium text-texto-suave">+</button>
            </div>
