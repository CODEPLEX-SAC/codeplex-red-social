...
/* ── Mini Reaction Popup — mantiene clase para animación y ::after ── */
...
/* ── Comment Menu (Edit/Delete) — mantiene clase para animación dropdown ── */
...
/* ── Single Comment ── */
function Comment({ comment, currentUser, modoExploracion, comenzarAutenticacion, onReply, onEdit, onDelete, onReaction, alVerPerfil }) {
  ...
  const isOwner = !modoExploracion && currentUser && currentUser.nombre === name;
  ...
  const handleLikeClick = () => {
    ...
    if (activeReaction) { onReaction(id, null); }
    else { onReaction(id, REACTIONS[0]); }
  };
  ...
  const handleEditCancel = () => { setEditText(text); setIsEditing(false); };
      ...
      <div className="flex gap-[10px] items-start">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            {/* comment-bubble mantiene clase para :has() selector */}
            <div className="comment-bubble">
              ...
              {isEditing ? (
                ...
                <div className="flex items-start gap-1">
                  <div className="text-[14px] text-[var(--text-dark)] leading-[1.5] mb-2">
            ...
            {isOwner && (
              <CommentMenu onEdit={() => { setEditText(text); setIsEditing(true); }} onDelete={() => onDelete(id)} />
            )}
          </div>
...
/* ── Íconos de acción (emoji, foto, gif, sticker) ── */
function CommentMediaIcons() {
  return (
    <div className="flex gap-[2px]">
      {[
        { title: "Emoji", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="12" r="10"/><path d="M8 14s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9" strokeWidth="3" strokeLinecap="round"/><line x1="15" y1="9" x2="15.01" y2="9" strokeWidth="3" strokeLinecap="round"/></svg> },
        { title: "Foto",  icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg> },
        { title: "GIF",   icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="2" y="4" width="20" height="16" rx="3"/><text x="12" y="15" textAnchor="middle" fill="currentColor" stroke="none" fontSize="8" fontWeight="700">GIF</text></svg> },
        { title: "Sticker",icon:<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 2a10 10 0 1 0 10 10h-10V2z"/><path d="M12 2v10h10"/></svg> },
      ].map(({ title, icon }) => (
        <button key={title} className="bg-transparent border-none cursor-not-allowed text-[var(--text-muted)] p-1 rounded-full flex items-center justify-center transition-colors duration-200 opacity-70 hover:text-[var(--text-dark)] hover:bg-[var(--hover-color)]" title={title} disabled>
          {icon}
        </button>
      ))}
    </div>
...
function updateCommentById(comments, id, updater) {
  return comments.map((c) => {
    ...
    if (c.replies && c.replies.length > 0) return { ...c, replies: updateCommentById(c.replies, id, updater) };
    return c;
  });
}
...
function removeCommentById(comments, id) {
  return comments
    .filter((c) => c.id !== id)
    .map((c) => {
      if (c.replies && c.replies.length > 0) return { ...c, replies: removeCommentById(c.replies, id) };
      return c;
    });
}
...
function Comentarios({ visible, alVerPerfil }) {
  ...
  const [comentarios, setComentarios] = useState(() =>
    MOCK_COMMENTS.map((c) => ({ ...c, id: genId(), replies: [], reaction: null, reactionCount: 0 }))
  );

  const handleSend = (texto) => {
    const nuevo = {
      id: genId(), img: 12, name: usuario?.nombre || "Gabriel Chumpitazi",
      text: texto, time: "Ahora", avatarUrl: usuario?.avatar || "https://i.pravatar.cc/150?img=12",
      replies: [], reaction: null, reactionCount: 0,
    };
  ...
  const handleReply = (parentId, texto) => {
    ...
    const findCommentName = (comments, id) => {
      for (const c of comments) {
        ...
        if (c.replies?.length) { const found = findCommentName(c.replies, id); if (found) return found; }
      }
      return null;
    };
    ...
    const reply = {
      ...
      replies: [], reaction: null, reactionCount: 0, mentionedUser,
    };
    setComentarios((prev) => updateCommentById(prev, parentId, (c) => ({ ...c, replies: [...(c.replies || []), reply] })));
  };

  const handleEdit   = (id, newText) => setComentarios((prev) => updateCommentById(prev, id, (c) => ({ ...c, text: newText })));
  const handleDelete = (id)          => setComentarios((prev) => removeCommentById(prev, id));
  ...
  const handleReaction = (commentId, reaction) => {
    setComentarios((prev) =>
      updateCommentById(prev, commentId, (c) => {
        ...
        if (c.reaction && c.reaction.label === reaction.label) return { ...c, reaction: null, reactionCount: Math.max(0, c.reactionCount - 1) };
  ...
  return (
    <div className="pt-4 flex flex-col gap-4">
      ...
      <CommentInput onSend={handleSend} modoExploracion={modoExploracion} comenzarAutenticacion={comenzarAutenticacion} currentUser={usuario} />
    </div>
  );
}
