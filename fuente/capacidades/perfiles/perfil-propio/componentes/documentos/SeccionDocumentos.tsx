...
export default function SeccionDocumentos({ perfilSocial, actualizarPerfilSocial, card, tituloLg }: Props) {
  ...
  const tieneTitulo = useMemo(() => documentosCargados.some((d) => d.tipo?.toLowerCase().includes("t�tulo") || d.nombre?.toLowerCase().includes("t�tulo")), [documentosCargados]);
    ...
    <div className={card}>
      ...
      <div className="flex flex-col gap-2">
        {documentosCargados.length ? documentosCargados.map((doc) => (
          ...
          <>
            ...
            <FilaVacia icono={<span>+</span>} label="+ Subir t�tulo profesional" sublabel="Ser� verificado por Codeplex" onClick={() => fileRef.current?.click()} />
          </>
        )}
      </div>
      ...
      {mostrarAviso && (
        <div className="flex items-center gap-2 mt-3 p-3 rounded-[var(--radius-sm)] border border-[#f59e0b] bg-[#fffbeb]">
          <p className="text-[12px] text-[#92400e] m-0">Debes subir al menos el CV y un t�tulo verificado para aparecer en b�squedas.</p>
        </div>
      )}
    </div>
  );
