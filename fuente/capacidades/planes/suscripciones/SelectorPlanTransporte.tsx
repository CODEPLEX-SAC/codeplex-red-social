...
import VisorCatalogoPDF from "@/compartido/interfaz/visualizacion-datos/VisorCatalogo/VisorCatalogoPDF";
import {
 PRECIOS_TRANSPORTE,
...
} from "@/capacidades/planes/suscripciones/transporte.data";
...
function SelectorPlanTransporte({ estaAbierto, alCerrar, alProcederPago }) {
 ...
 <div className="bg-surface rounded-[var(--radius-xl)] w-full max-w-[760px] max-h-[90vh] overflow-hidden relative flex flex-col [animation:modal-slide_0.28s_cubic-bezier(0.34,1.56,0.64,1)] shadow-[0_24px_80px_rgba(0,0,0,0.22)]">
 ...
 <h2 className="text-[22px] font-extrabold text-fg m-0 max-[700px]:text-[19px]">Gestión-Plex Transporte</h2>
 ...
 <span className="border-[1.5px] border-[#BFDBFE] rounded-full px-3.5 py-1 text-xs font-medium bg-surface text-[#1D4ED8]">1 mes gratis</span>
 <span className="border-[1.5px] border-[#BFDBFE] rounded-full px-3.5 py-1 text-xs font-medium bg-surface text-[#1D4ED8]">IGV incluido</span>
 <span className="border-[1.5px] border-[#BFDBFE] rounded-full px-3.5 py-1 text-xs font-medium bg-surface text-[#1D4ED8]">Actualizaciones</span>
 <span className="border-[1.5px] border-[#BFDBFE] rounded-full px-3.5 py-1 text-xs font-medium bg-surface text-[#1D4ED8]">Usuarios ilimitados</span>
 </div>

 <div className="inline-flex bg-base rounded-[var(--radius-sm)] p-1 gap-1 mt-1">
 <button
 className={`px-6 py-2 rounded-lg border-0 text-[13.5px] font-semibold cursor-pointer transition-all outline-none${periodoFacturacion === "mensual" ? " bg-[#1E3A8A] text-white shadow-[0_2px_8px_rgba(30,58,138,0.35)]" : " bg-transparent text-muted"}`}
 onClick={() => setPeriodoFacturacion("mensual")}
 >Mensual</button>
 <button
 className={`px-6 py-2 rounded-lg border-0 text-[13.5px] font-semibold cursor-pointer transition-all outline-none${periodoFacturacion === "anual" ? " bg-[#1E3A8A] text-white shadow-[0_2px_8px_rgba(30,58,138,0.35)]" : " bg-transparent text-muted"}`}
 ...
 {/* ── Plan Gratis ── */}
 <div className={`bg-surface border-[1.5px] border-line rounded-2xl px-4 pt-5 pb-[18px] flex flex-col gap-3 transition-[box-shadow,border-color] duration-[180ms] relative flex-[0_0_210px] min-w-[210px] hover:shadow-[0_4px_20px_rgba(0,0,0,0.08)]${planSeleccionado === "gratis" ? " border-2 border-[#1D4ED8] shadow-[0_4px_20px_rgba(29,78,216,0.18)]" : ""}`}>
 ...
 <li key={f} className="flex items-start gap-1.5 text-xs text-fg leading-[1.4] cursor-pointer rounded-md px-1 py-0.5 -mx-1 transition-colors hover:bg-[rgba(29,78,216,0.08)]" onClick={() => setFeatureAbierta(f)}>
 <IconoCheck />
 <span>{f}</span>
 </li>
 ))}
 ...
 <button
 className={`outline-none w-full py-[10px] rounded-[var(--radius-sm)] text-[13.5px] font-bold cursor-pointer transition-all mt-auto${planSeleccionado === "gratis" ? " bg-[#1D4ED8] text-white border-0" : " bg-transparent border-2 border-[#1D4ED8] text-[#1D4ED8] hover:bg-[#1D4ED8] hover:text-white"}`}
 ...
 <button
 className={`outline-none w-full py-[10px] rounded-[var(--radius-sm)] text-[13.5px] font-bold cursor-pointer transition-all mt-auto${planSeleccionado === "transporte" ? " bg-white/15 border-0 text-white" : " bg-transparent border-2 border-white/70 text-white hover:bg-white/15 hover:border-white"}`}
 ...
 {/* Hint chat */}
 <div className="flex items-center justify-end gap-1.5 mt-3.5 text-[13px] text-muted cursor-pointer hover:text-[#1D4ED8]" onClick={() => setChatVisible(true)}>
 ...
 <p className={`m-0 px-3 py-[9px] text-[12.5px] leading-[1.5] whitespace-pre-line${msg.tipo === "bot" ? " bg-surface text-fg rounded-[4px_14px_14px_14px] shadow-[0_1px_3px_rgba(0,0,0,0.07)]" : " bg-[#1D4ED8] text-white rounded-[14px_4px_14px_14px]"}`}>{msg.texto}</p>
 ...
 <button key={q} className="text-[11px] font-medium px-[11px] py-[5px] rounded-full border-[1.5px] cursor-pointer transition-all whitespace-nowrap border-[#BFDBFE] bg-surface text-[#1D4ED8] hover:bg-[#1D4ED8] hover:border-[#1D4ED8] hover:text-white" onClick={() => handleEnviarChat(q)}>
 {q}
 </button>
 ))}
 </div>
 ...
 <input
 className="flex-1 border-[1.5px] border-line rounded-full px-3.5 py-2 text-[12.5px] outline-none bg-base text-fg transition-colors placeholder:text-subtle focus:border-[#1D4ED8]"
