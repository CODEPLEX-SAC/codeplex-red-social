...
import {
 ...
 FEATURES_COMERCIAL,
} from "@/capacidades/planes/suscripciones/gestionplex.data";
...
function PlanCard({ plan, periodo, seleccionado, onSeleccionar, onFeatureClick }) {
 ...
 <div
 className={[
 "bg-surface border-[1.5px] border-line rounded-2xl px-4 pt-5 pb-[18px] flex flex-col gap-3 transition-[box-shadow,border-color] duration-[180ms] relative flex-[0_0_192px] min-w-[192px] hover:shadow-[0_4px_20px_rgba(0,0,0,0.08)]",
 ...
 <div className={`flex items-baseline gap-0.5${plan.dark ? " text-white" : " text-fg"}`}>
 ...
 <li
 key={i}
 className={`flex items-start gap-1.5 text-xs leading-[1.4]${plan.dark ? " text-white" : " text-fg"}${f.check === false ? " opacity-80" : ""}`}
 >
 {f.check === true
 ...
 >
 {f.check === false ? <span className="line-through text-subtle text-[11.5px]">{f.texto}</span> : (plan.dark && FEATURES_COMERCIAL[f.texto] ? <u>{f.texto}</u> : f.texto)}
 </span>
 </li>
 ))}
 </ul>
...
function SelectorPlanGestionPlex({ estaAbierto, alCerrar, alProcederPago }) {
 ...
 {/* Botón volver */}
 <button className="inline-flex items-center gap-1.5 bg-transparent border-0 text-[13px] font-semibold text-fg cursor-pointer px-2 py-1 rounded-lg mb-1.5 transition-colors hover:bg-black/[0.06] outline-none focus:outline-none" onClick={handleClose}>
 ...
 <h2 className="text-[22px] font-extrabold text-fg m-0 max-[700px]:text-[19px]">Gestión-Plex Comercial</h2>
 ...
 <span className="border-[1.5px] border-[#99F6E4] rounded-full px-3.5 py-1 text-xs font-medium bg-surface text-[#0F766E]">1 mes gratis</span>
 <span className="border-[1.5px] border-[#99F6E4] rounded-full px-3.5 py-1 text-xs font-medium bg-surface text-[#0F766E]">IGV incluido</span>
 <span className="border-[1.5px] border-[#99F6E4] rounded-full px-3.5 py-1 text-xs font-medium bg-surface text-[#0F766E]">Actualizaciones</span>
 </div>
 ...
 <button
 className={`px-6 py-2 rounded-lg border-0 text-[13.5px] font-semibold cursor-pointer transition-all outline-none focus:outline-none${periodoFacturacion === "anual" ? " bg-[#134E4A] text-white shadow-[0_2px_8px_rgba(19,78,74,0.35)]" : " bg-transparent text-muted"}`}
 ...
 {/* Hint chat */}
 <div className="flex items-center justify-end gap-1.5 mt-3.5 text-[13px] text-muted cursor-pointer hover:text-[#0D9488]" onClick={() => setChatVisible(true)}>
 ...
 <p className={`m-0 px-3 py-[9px] text-[12.5px] leading-[1.5] whitespace-pre-line${msg.tipo === "bot" ? " bg-surface text-fg rounded-[4px_14px_14px_14px] shadow-[0_1px_3px_rgba(0,0,0,0.07)]" : " bg-[#0D9488] text-white rounded-[14px_4px_14px_14px]"}`}>{msg.texto}</p>
 ...
 <button key={q} className="text-[11px] font-medium px-[11px] py-[5px] rounded-full border-[1.5px] cursor-pointer transition-all whitespace-nowrap border-[#99F6E4] bg-surface text-[#0F766E] hover:bg-[#0D9488] hover:border-[#0D9488] hover:text-white" onClick={() => handleEnviarChat(q)}>
 {q}
 </button>
 ))}
 </div>
 ...
 <input
 className="flex-1 border-[1.5px] border-line rounded-full px-3.5 py-2 text-[12.5px] outline-none bg-base text-fg transition-colors placeholder:text-subtle focus:border-[#0D9488]"
