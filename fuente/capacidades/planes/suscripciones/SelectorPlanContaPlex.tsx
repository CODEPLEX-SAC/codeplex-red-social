...
import {
 ...
 CHATBOT_MSG_INICIAL,
 FEATURES_INFO,
} from "@/capacidades/planes/suscripciones/contaplex.data";
...
function PopoverDetalleFeature({ nombreFeature, onCerrar }) {
 ...
 >
 <button
 className="absolute top-3 right-3 w-6 h-6 rounded-full bg-base border-none cursor-pointer flex items-center justify-center transition-colors duration-150 hover:bg-line"
 onClick={onCerrar}
 >
 ...
 <h4 className="text-[15px] font-bold text-fg m-0 mb-[10px] pr-5 leading-[1.3]">{info.titulo}</h4>
...
function PlanCard({ plan, periodo, seleccionado, onSeleccionar, compact = false, onFeatureClick }) {
 ...
 <div className={`flex items-baseline gap-[2px] ${plan.dark ? "text-white" : "text-fg"}`}>
 ...
 <li
 key={i}
 className={`flex items-start gap-[6px] text-[12px] leading-[1.4] ${plan.dark ? "text-white" : "text-fg"}`}
 >
 ...
 <span
 className={[
 f.check === false ? (plan.dark ? "line-through text-white/40 text-[11.5px]" : "line-through text-subtle text-[11.5px]") : "",
...
function SelectorPlanContaPlex({ estaAbierto, alCerrar, alProcederPago }) {
 ...
 <h2 className="text-[20px] font-extrabold text-fg m-0 max-[700px]:text-[19px]">{tituloPlanes}</h2>
 ...
 <small className={`text-[11px] transition-colors duration-[0.18s] ${tipoSeleccionado === tab.key ? "text-white/75" : "text-subtle"}`}>{tab.sub}</small>
 </button>
 ))}
 </div>
 ...
 <span className={`text-[13px] font-medium transition-colors duration-[0.18s] ${periodoFacturacion === "mensual" ? "text-fg font-bold" : "text-subtle"}`}>Mensual</span>
 <button
 ...
 >
 <span
 className="absolute top-[3px] left-[3px] w-[18px] h-[18px] bg-surface rounded-full shadow-[0_1px_4px_rgba(0,0,0,0.2)] transition-transform duration-[0.22s] [transition-timing-function:cubic-bezier(0.34,1.56,0.64,1)] block"
 style={{ transform: periodoFacturacion === "anual" ? "translateX(18px)" : "translateX(0)" }}
 />
 </button>
 <span className={`text-[13px] font-medium transition-colors duration-[0.18s] ${periodoFacturacion === "anual" ? "text-fg font-bold" : "text-subtle"}`}>Anual</span>
 </div>
 </div>
 ...
 {/* ── Hint chat ── */}
 <div
 className="flex items-center justify-end gap-[6px] mt-[14px] text-[13px] text-muted cursor-pointer hover:text-[#0094C0]"
 ...
 return (
 <div className="flex items-center gap-[10px] px-[14px] py-[10px] border-b border-line-light bg-base flex-shrink-0 cursor-pointer" onClick={abrirVisor}>
 <IconoPdf />
 ...
 {/* Mensajes */}
 <div className="flex-1 overflow-y-auto px-3 py-[14px] flex flex-col gap-[10px] bg-base [&::-webkit-scrollbar]:w-[4px] [&::-webkit-scrollbar-thumb]:bg-line [&::-webkit-scrollbar-thumb]:rounded-[99px]">
 ...
 {/* Quick replies */}
 <div className="flex flex-wrap gap-[6px] px-3 py-2 border-t border-line-light bg-surface flex-shrink-0 max-[700px]:flex-wrap max-[700px]:gap-[6px] max-[700px]:pb-3">
 {CHATBOT_QUICK.map((q) => (
 <button
 key={q}
