...
function LanguageSelect() {
  ...
  return (
    <div className="custom-select-wrapper relative min-w-[110px]">
      <button
        className="w-full flex items-center justify-between gap-2 px-[10px] py-[6px] border border-[var(--border-color)] rounded-[var(--radius-xs)] text-[13px] text-[var(--text-dark)] bg-[var(--white-color)] cursor-pointer outline-none focus:border-[var(--primary-color)]"
      ...
      {open && (
        <ul className="absolute top-[calc(100%+4px)] left-0 right-0 bg-[var(--white-color)] border border-[var(--border-color)] rounded-[var(--radius-xs)] overflow-hidden z-[200] list-none m-0 p-0 shadow-[0_4px_12px_rgba(0,0,0,0.1)]">
          {options.map((opt) => (
            <li
              key={opt}
...
function ProfileSlider({ alCerrar, darkMode, onToggleDarkMode, alIniciarSesion, alCerrarSesion, alVerPerfil }) {
  ...
  return (
    <>
      <div className="animate-slider-overlay fixed inset-0 bg-[rgba(0,0,0,0.5)] z-[1000]" onClick={alCerrar} />
      <div className="animate-slider-cuenta fixed top-0 right-0 w-full [@media(min-width:481px)]:w-[300px] h-screen bg-[var(--white-color)] shadow-[-2px_0_20px_rgba(0,0,0,0.15)] flex flex-col z-[1001] overflow-y-auto">
        ...
        <div className="px-5 py-[18px] text-white flex items-center justify-between shrink-0" style={{ background: 'var(--gradient-primary)' }}>
          ...
          <button
            className="bg-transparent border-none text-white text-[22px] cursor-pointer w-7 h-7 flex items-center justify-center rounded-[var(--radius-xs)] transition-[background] duration-300 leading-none hover:bg-[rgba(255,255,255,0.2)]"
            onClick={alCerrar}
          >
            ×
          </button>
        ...
        <div className="px-5 pt-6 pb-4 text-center shrink-0 bg-[var(--white-color)]">
          {modoExploracion ? (
            <>
              ...
              <div className="bg-gradient-to-br from-[#7F0DF2] to-[#588CE5] rounded-[var(--radius-md)] p-4 mt-[14px] mb-2 text-center">
                ...
                <button
                  className="bg-[var(--surface-color)] text-[#7F0DF2] border-none px-5 py-2 rounded-[var(--radius-lg)] text-[13px] font-bold cursor-pointer transition-opacity duration-150 hover:opacity-[0.85]"
        ...
        <div className="flex flex-col gap-[10px] p-5 shrink-0 bg-[var(--white-color)]">
          {!modoExploracion && (
            <>
              ...
              <button
                className="bg-transparent border-[1.5px] border-[var(--error-color)] p-[13px] rounded-[var(--radius-sm)] text-[var(--error-color)] font-semibold text-[15px] cursor-pointer flex items-center justify-center gap-2 transition-all duration-300 w-full hover:bg-[var(--error-color)] hover:text-white"
